...
namespace scc::lowering::inbuild
{
    ...
    InterpreterResult sinf(InterpreterState &state)
    {
        ...
        if (value.get_value().type != Type(Type::Kind::F32))
            return InterpreterError::ReachedUnreachableError;
        
        if (not std::holds_alternative<Type::Primitive::F32>(value.get_value().value))
            return InterpreterError::ReachedUnreachableError;
        
        Type::Primitive::F32 val = std::get<Type::Primitive::F32>(value.get_value().value);
    ...
    InterpreterResult sin(InterpreterState &state)
    {
        ...
        if (value.get_value().type != Type(Type::Kind::F64))
            return InterpreterError::ReachedUnreachableError;
        
        if (not std::holds_alternative<Type::Primitive::F64>(value.get_value().value))
            return InterpreterError::ReachedUnreachableError;
        
        Type::Primitive::F64 val = std::get<Type::Primitive::F64>(value.get_value().value);
